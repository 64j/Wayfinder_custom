...
class Wayfinder {
	...
	function checkTemplates() {
		...
		if(!empty($nonWayfinderFields)) {
			$this->tvList = array_unique($nonWayfinderFields);
			if($this->_config['debug']) {
	...
	function findTemplateVars($tpl) {
		...
		foreach($matches[1] as $tv) {
			if(strpos(strtolower($tv), 'phx') === 0) {
				continue;
			}
			if(strpos(strtolower($tv), 'wf.') === 0) {
				continue;
			}
			$tvnames[] = $tv;
		}
	...
	function getData() {
		...
		if(!$this->_config['hideSubMenus']) {
			...
			if($this->_config['id'] == 0 || in_array($this->_config['id'], $parents)) {
				...
				foreach($parents as $p) $ids = $modx->getChildIds($p, 1, $ids);
			}
		}
		...
		if(!empty($ids)) {
			...
			foreach($fields as $i => $v) {
				if($v == 'alias') {
					$fields[$i] = "IF(sc.alias='', sc.id, sc.alias) AS alias";
				} elseif($v == 'content') {
					$fields[$i] = "IF(sc.type='reference',sc.content,'') AS content";
				} else {
					$fields[$i] = 'sc.' . $v;
				}
			}
			...
			if($modx->isFrontend()) {
				...
				if($docgrp) {
					$access .= sprintf(' OR dg.document_group IN (%s)', $docgrp);
				}
			}
			if($access) {
				$access = "AND({$access})";
			...
			if($this->_config['includeDocs']) {
				$menuWhere .= sprintf(' AND sc.id IN (%s)', $this->_config['includeDocs']);
			}

			//add the exclude docs to the where clause
			if($this->_config['excludeDocs']) {
				$menuWhere .= sprintf(' AND (sc.id NOT IN (%s))', $this->_config['excludeDocs']);
			}

			//add custom where conditions
			if(!empty($this->_config['where'])) {
				$menuWhere .= sprintf(' AND (%s)', $this->_config['where']);
			}

			//add the limit to the query
			if($this->_config['limit']) {
				$limit = sprintf('0, %s', $this->_config['limit']);
			} else {
				$limit = '';
			}
			...
			while($row = $modx->db->getRow($result)) {
				...
				if($this->_config['useWeblinkUrl'] && $row['type'] == 'reference') {
					if(preg_match('@^[1-9][0-9]*$@', $row['content'])) {
						$row['link'] = $modx->makeUrl(intval($row['content']), '', '', $linkScheme);
					} else {
			...
			if(!empty($this->tvList) && !empty($resultIds)) {
				...
				$tvValues = $this->appendTV($this->tvList, $resultIds);
	...
	function appendTV($tvs, $docIDs) {
		global $modx;

		$resourceArray = array();

		$sql = $modx->db->query('SELECT tv.name, tvc.contentid, tvc.value 
			FROM modx_site_tmplvars AS tv
			LEFT JOIN modx_site_tmplvar_contentvalues AS tvc ON tv.id=tvc.tmplvarid
			WHERE tvc.contentid IN(' . implode(',', $docIDs) . ') AND tv.name IN("' . implode('","', $tvs) . '")');

		foreach($modx->db->makeArray($sql) as $key => $value) {
			$resourceArray['#' . $value['contentid']][$value['name']] = $value['value'];
		}
		return $resourceArray;
	}
	...
	function buildSubMenu($subDocs, $level) {
		...
		if($this->_templates['innerTpl'] && $wrapperClass == 'innercls') {
			if(!empty($this->_templates['innerTpl' . $level])) {
				$tpl = $this->_templates['innerTpl' . $level];
			} else {
	...
	function renderRow(&$resource, $numChildren, $curNum) {
		...
		if($this->_config['displayStart'] && $resource['level'] == 0) {
		...
		} elseif($resource['id'] == $modx->documentIdentifier && $resource['isfolder'] && $this->_templates['parentRowHereTpl'] && ($resource['level'] < $this->_config['level'] || $this->_config['level'] == 0) && $numChildren) {
			if(!empty($this->_templates['parentRowHereTpl' . $resource['level']])) {
				$usedTemplate = 'parentRowHereTpl' . $resource['level'];
			} else {
				$usedTemplate = 'parentRowHereTpl';
			}
		} elseif($resource['id'] == $modx->documentIdentifier && $this->_templates['innerHereTpl'] && $resource['level'] > 1) {
			if(!empty($this->_templates['innerHereTpl' . $resource['level']])) {
				$usedTemplate = 'innerHereTpl' . $resource['level'];
			} else {
				$usedTemplate = 'innerHereTpl';
			}
		} elseif($resource['id'] == $modx->documentIdentifier && $this->_templates['hereTpl']) {
			if(!empty($this->_templates['hereTpl' . $resource['level']])) {
				$usedTemplate = 'hereTpl' . $resource['level'];
			} else {
			...
			if(!empty($this->_templates['activeParentRowTpl' . $resource['level']])) {
				$usedTemplate = 'activeParentRowTpl' . $resource['level'];
			} else {
			...
			if(!empty($this->_templates['categoryFoldersTpl' . $resource['level']])) {
				$usedTemplate = 'categoryFoldersTpl' . $resource['level'];
			} else {
			...
			if(!empty($this->_templates['parentRowTpl' . $resource['level']])) {
				$usedTemplate = 'parentRowTpl' . $resource['level'];
			} else {
				$usedTemplate = 'parentRowTpl';
			}
		} elseif($resource['level'] > 1 && $this->_templates['innerRowTpl']) {
			if(!empty($this->_templates['innerRowTpl' . $resource['level']])) {
				$usedTemplate = 'innerRowTpl' . $resource['level'];
			} else {
				$usedTemplate = 'innerRowTpl';
			}
		} elseif($resource['last'] && $this->_templates['lastRowTpl']) {
			if(!empty($this->_templates['lastRowTpl' . $resource['level']])) {
				$usedTemplate = 'lastRowTpl' . $resource['level'];
			} else {
				$usedTemplate = 'lastRowTpl';
			}
		} else {
			if(!empty($this->_templates['rowTpl' . $resource['level']])) {
				$usedTemplate = 'rowTpl' . $resource['level'];
			} else {
				$usedTemplate = 'rowTpl';
			}
		}
	...
	function setItemClass($classType, $docId = 0, $first = 0, $last = 0, $level = 0, $isFolder = 0, $type = 'document') {
		...
		switch($classType) {
			case 'outercls':
				if(!empty($class['outer'])) {
					$classNames[] = $class['outer'];
				}             //Set outer class
				break;
			case 'innercls':
				if(!empty($class['inner'])) {
					$classNames[] = $class['inner'];
				}             //Set inner class
				if(!empty($class['outerLevel'])) {
			...
			case 'rowcls':
				...
				if($first && !empty($class['first'])) {
					$classNames[] = $class['first'];
				}             //Set first class
				if($last && !empty($class['last'])) {
				...
				if(!empty($class['here']) && $this->isHere($docId)) {
					$classNames[] = $class['here'];
				}              //Set here class
				if(!empty($class['self']) && $docId == $config['hereId']) {
					$classNames[] = $class['self'];
				}              //Set self class
				if(!empty($class['weblink']) && $type == 'reference') {
